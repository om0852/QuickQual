...
export function useTetris() {
  const { tetriesScore, setTeterisScore, startState } =
  ...
  const startGame = useCallback(() => {
    ...
    setTeterisScore((prev) => prev - 1);
    localStorage.setItem("tetris", score.toString());
  ...
  const commitPosition = useCallback(() => {
    ...
    setTeterisScore((prevScore) => prevScore + getPoints(numCleared));
    localStorage.setItem("tetris", score.toString());
    dispatchBoardState({
      type: "commit",
